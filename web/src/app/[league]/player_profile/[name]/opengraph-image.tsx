...
export const alt = 'LeagueScope Player Profile';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default async function Image({ params }: { params: { league: string; name: string } }) {
  const playerName = decodeURIComponent(params.name).slice(0, 30);
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ position: 'absolute', top: '-150px', left: '50%', width: '600px', height: '600px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,0.2) 0%, transparent 60%)', transform: 'translateX(-50%)', display: 'flex' }} />
        {/* Label */}
        ...
        {/* Player name */}
        <span style={{ fontSize: playerName.length > 12 ? '80px' : '100px', fontWeight: 900, color: '#ffffff', letterSpacing: '-3px', lineHeight: 1 }}>
          {playerName}
        </span>
        {/* League badge */}
