...
import DragonChart from '../overview/DragonChart';
import { useFilters } from '@/context/FilterContext';
import type { OverviewData } from '../overview/OverviewClient';

/* ═══════════════════════════════════════════════════════════════════════════
   Pruebas50 — Overview restyled with home-card aesthetic
   ═══════════════════════════════════════════════════════════════════════════ */
...
export default function Pruebas50Client({ league, accent, initialData }: Props) {
  ...
  if (!data || !data.tournament) {
    return (
      <div className="p50-container" style={{ '--p50-accent': accent } as React.CSSProperties}>
        <div className="p50-header">
          <div className="p50-header-inner">
            <Image src={LEAGUE_LOGO(league)} alt={league} width={48} height={48} className="p50-header-logo" />
            <div>
              <h1 className="p50-header-title">{league.toUpperCase()} OVERVIEW</h1>
            </div>
          </div>
        </div>
    ...
    <div className="p50-container" style={{ '--p50-accent': accent } as React.CSSProperties}>
      ...
      <div className="p50-header">
        <div className="p50-header-inner">
          ...
          <div>
            <h1 className="p50-header-title">{league.toUpperCase()} OVERVIEW</h1>
            <div className="p50-header-sub">SEASON {filters.year || ''} // {(filters.split || '').toUpperCase()}</div>
          </div>
        </div>
      </div>
